...
class Solution {
    ...
    struct Step {
        ...
        Step(size_t row, size_t column, int timeFromStart) :
                row{ row }, column{ column }, timeFromStart{ timeFromStart } {};
    };
...
private:
    int dijkstraSearchForPathWithMinTime(span<const vector<int>> moveTime) const {
        ...
        minHeapForTime.emplace(startRow, startColumn, 0);
        ...
        while (!minHeapForTime.empty()) {
            ...
            for (const auto& move : MOVES) {
                ...
                int nextValueForTime = max(1 + current.timeFromStart, 1 + moveTime[nextRow][nextColumn]);
                if (minTimeMatrix[nextRow][nextColumn] > nextValueForTime) {
                    minTimeMatrix[nextRow][nextColumn] = nextValueForTime;
                    minHeapForTime.emplace(nextRow, nextColumn, nextValueForTime);
                }
            }
        }
