...
class Solution {
    ...
    private data class Step(val row: Int, val column: Int, val timeFromStart: Int) {}
    ...
    private fun dijkstraSearchForPathWithMinTime(moveTime: Array<IntArray>): Int {
        ...
        minHeapForTime.add(Step(startRow, startColumn, 0))
        ...
        while (!minHeapForTime.isEmpty()) {
            ...
            for (move in MOVES) {
                ...
                val nextValueForTime = max(1 + current.timeFromStart, 1 + moveTime[nextRow][nextColumn])
                if (minTimeMatrix[nextRow][nextColumn] > nextValueForTime) {
                    minTimeMatrix[nextRow][nextColumn] = nextValueForTime
                    minHeapForTime.add(Step(nextRow, nextColumn, nextValueForTime))
                }
            }
        }
